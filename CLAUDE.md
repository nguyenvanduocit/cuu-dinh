...
| 26 | **Tech stack web-first**: Vue 3 + PixiJS v8 + Rapier 2D + Nakama + Vite + Bun monorepo. Hybrid DOM (meta UI 80%) + Canvas (match layer 20%). Supersedes Godot 4 decision cùng ngày. Form-factor = mobile-only portrait (xem #31) | 2026-05-19 |
...
| 28 | **Game title = "Cửu Đỉnh"** (Nine Cauldrons). Working title "Lư Đan" retired. Lore weave: 9 đỉnh thượng cổ rèn bởi Cao Tổ + 7 đan sư + Lò Thần. 8 đỉnh vỡ rải khắp atlas sau Cửu Chuyển thất bại; đỉnh thứ 9 (trung lập) sống sót = Lò player kế thừa. Game title = quest tổng thể: tái hợp đủ 9 đỉnh. Package npm scope `@cuu-dinh/*`, GitHub repo `cuu-dinh`. ⚠ **Title + lore weave "9 đỉnh" superseded by #30** — chỉ phần npm scope + repo slug còn áp dụng. | 2026-05-20 |
| 29 | **Concept-phase content authoring**: per-entity folder bundle trong `docs/content/<category>/<entity-id>/` — mỗi entity 1 folder = SSOT với `design.md` (YAML frontmatter machine-readable + markdown mechanics) + `lore.md` + `prompt.md` + `art/` (optional khi có content). Cross-entity table view = derived (`_catalog.md` regenerate qua `tools/derive-catalog.ts`). 22 category bao trùm item-shaped (items, uniques, affixes, equipment, currency, passive-tree, ascendancies, maps, sets, la-han), character-shaped (npcs, bosses, factions, lore-entities), narrative-shaped (acts với nested nodes), spatial (regions, arenas), UI/visual (screens unified), audio (tracks bundled, sfx table-exception), effects/icons. Cross-cutting design docs (combat-math, economy-flow, progression-curve, lore, art-direction, audio-spec, accessibility) sống ở `docs/content/*.md` cùng cấp per-entity bundle; process/meta docs (tech-stack, roadmap, onboarding, ui-wireframes) ở `docs/*.md`. Implementation phase: Zod schema + JSON regenerate từ `design.md` frontmatter. Spec: `docs/superpowers/specs/2026-05-20-per-entity-content-bundle-design.md` | 2026-05-20 / revised 2026-05-20 (lần 2) |
| 30 | **Game title = "Đan Kiếp"** (丹劫 — Alchemy Calamity / kiếp nạn của đan đạo). "Cửu Đỉnh" retired; working-title lineage: Lư Đan → Cửu Đỉnh → **Đan Kiếp**. Lý do: tên khớp campaign đã viết — Cửu Chuyển Kim Đan thất bại = trận **Đan Kiếp** lớn nhất lịch sử (sinh ra Hỗn Nguyên Tâm Ma Vương), và mỗi linh đan player luyện đều mang rủi ro đan kiếp riêng (bể đan, thiên kiếp). Supersedes **title + lore weave "9 đỉnh thượng cổ" của #28**: concept 9 đỉnh gỡ bỏ — review storyline 2026-05-20 xác nhận nó chưa từng được dệt vào `lore.md` timeline hay 5-act campaign (orphaned). Lò player = Lò Luyện Đan tổ truyền độc lập; bí ẩn của Lò dồn vào Cửu Chuyển + tiếng đếm "chuyển thứ chín". `docs/content/lore-entities/` 9 file cauldron + `neutral-cauldron` cần dọn/viết lại. **Giữ nguyên** npm scope `@cuu-dinh/*` + GitHub repo `cuu-dinh` (slug kỹ thuật ≠ display title; đổi repo phá link). Tên lục địa vẫn "Cửu Châu Đan Vực"; chỗ NPC dialogue dùng "Cửu Đỉnh" làm tên thế giới → đổi sang "Cửu Châu Đan Vực". | 2026-05-20 |
| 31 | **Form-factor = mobile-only portrait (dọc)**, chạy mobile browser (PWA-ready). Driver: chơi-1-tay + thẩm mỹ 立軸 (hanging scroll). Touch primary, viewport ~390×844. UI khung 3 vùng (top status / 立軸 content / bottom thumb-bar) + bottom-sheet detail; nguyên tắc "số liệu ≤1 tap", "thumb-zone, touch target ≥44px". 13 screen wireframe ở `docs/content/screens/<id>/`. Graph nav (tree/atlas semantic zoom) + combat "Thác Tầng Dọc" cần playtest sớm. Spec: `docs/superpowers/specs/2026-05-20-mobile-portrait-pivot-design.md` | 2026-05-20 |
| 32 | **CLAUDE.md cascade hierarchy trong `docs/content/`** = **system-prompt extension by depth**, KHÔNG phải file-format doc. Mỗi tier load domain context + design lens + lore voice phù hợp depth. 3-tier: Tier 1 (`docs/content/CLAUDE.md` = Đan Kiếp universe, vocabulary cốt lõi, ngũ hành cosmology, design philosophy, lore voice, 3-layer + 6-trụ gambling surface) + Tier 2 (`docs/content/<cat>/CLAUDE.md` cho cả 22 category = category là gì trong universe, design principles cho category, lore voice nuance, cross-system relationship) + Tier 3 (`docs/content/<cat>/<sub>/CLAUDE.md` chỉ 4 large category >50 entity = sub-type lore + design constraints. Axis: `items` by item-type, `affixes` by prefix/suffix/implicit, `passive-tree` by Ngũ Hành element, `currency` by category). Tightness: Tier N không lặp Tier N-1. **File-format / folder rule / frontmatter schema KHÔNG ở CLAUDE.md** — đó là `README.md` (human nav) + per-entity bundle spec (`docs/superpowers/specs/2026-05-20-per-entity-content-bundle-design.md`). Migration phased B1 (Tier 1 + #32 lock — done) → B2 (Tier 2 cho 21 cat — done) → B3 (Tier 3 27 file + folder restructure 4 large via `tools/restructure-large-categories.ts` — done 2026-05-23). Spec: `docs/superpowers/specs/2026-05-23-claude-md-cascade-hierarchy-design.md` | 2026-05-23 |

Full context: `docs/superpowers/specs/2026-05-19-cuu-dinh-master-design.md` + `docs/onboarding.md` + `docs/superpowers/specs/2026-05-19-tech-stack-revisit.md` + `docs/superpowers/specs/2026-05-20-naming-convention-pivot.md` + `docs/superpowers/specs/2026-05-20-per-entity-content-bundle-design.md` + `docs/superpowers/specs/2026-05-23-claude-md-cascade-hierarchy-design.md`.
...
- Hardcode item/affix/map/passive data. Concept phase: content sống trong `docs/content/<category>/<entity-id>/` per-entity bundle (SSOT). Implementation phase: regenerate structured JSON + Zod schema từ `design.md` frontmatter.
- Edit `_catalog.md` bằng tay. File này regenerate từ folder qua `tools/derive-catalog.ts`. Edit folder con thay vì file catalog.
...
- Concept-phase content sống trong `docs/content/<category>/<entity-id>/` — per-entity bundle với `design.md` + `lore.md` + `prompt.md` + `art/`. `design.md` frontmatter (YAML) là machine-readable contract — column spec định nghĩa enum vocabulary tại `content/enums.md`. Structured JSON + Zod schema regenerate từ frontmatter khi vào implementation phase.
- Khi add entity mới: tạo folder `content/<cat>/<id>/`, viết `design.md` với frontmatter đầy đủ, tuỳ chọn thêm `lore.md`/`prompt.md`/`art/`. Run `bun tools/derive-catalog.ts <cat>` để rebuild catalog.
...
│   │   └── 2026-05-20-per-entity-content-bundle-design.md  # ⭐ Per-entity bundle spec
│   ├── content/                    # ⭐ Design content — per-entity bundle + cross-cutting design docs
│   │   ├── enums.md                # Shared vocabulary registry (enum + quy ước bảng)
│   │   ├── style/                  # Shared MJ art tokens (convention, palette, element/rarity)
│   │   ├── README.md               # Cấu trúc bundle + cách thêm entity
│   │   ├── combat-math.md / economy-flow.md / progression-curve.md   # Cross-cutting design docs
│   │   ├── lore.md / art-direction.md / audio-spec.md / accessibility.md   #   (sống cùng cấp per-entity)
│   │   └── <category>/<id>/        # 22 category: items, uniques, affixes, equipment, currency,
│   │                               #   passive-tree, ascendancies, maps, sets, la-han,
│   │                               #   npcs, bosses, factions, lore-entities, acts,
│   │                               #   regions, arenas, screens, tracks, sfx, effects, icons
│   │                               #   Mỗi <id>/ = design.md + optional lore.md / prompt.md / art/
│   └── *.md                        # Process/meta docs: tech-stack, roadmap, onboarding, ui-wireframes
├── apps/                           # (kế hoạch)
│   ├── web/                        # Vue + Pixi client
│   │   ├── package.json
│   │   ├── vite.config.ts
...
- **Content phase**: edit `docs/content/<cat>/<id>/{design,lore,prompt}.md` per-entity. AI có thể dispatch parallel: 1 agent = 1 folder, zero merge conflict. Frontmatter là contract; `content/enums.md` định nghĩa enum dùng chung. Run `tools/derive-catalog.ts <cat>` để regenerate catalog sau khi batch edit. Structured JSON + Zod regenerate ở implementation phase.
