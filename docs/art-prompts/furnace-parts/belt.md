> Nguồn data: docs/data/equipment.md (slot=belt). Append [STYLE SUFFIX] từ _style/style-suffix.md.

# Furnace Parts — Belt (Dây Phong Ấn)

STYLE SUFFIX = `moody pixel art game art, xianxia daoist alchemy aesthetic, talismanic cinnabar runes and bagua trigram motifs, limited six-color palette of ink black, cinnabar red, imperial gold, ivory white, jade green and twilight purple, hand-crafted sprite craftsmanship in the spirit of Eastward and Sea of Stars, atmospheric serious mystical tone, crisp hard pixel edges --style raw --v 6 --no anime, manga, ukiyo-e, western fantasy, generic asian temple stock photo, photorealistic, 3d render, glossy plastic, modern, text, ui, watermark, signature, blur`

---

### `belt-common` — Dây Phong Ấn Phàm

**Mô tả**: Dây phong ấn vải thô bình thường quấn quanh lò, nút thắt talisman đơn giản. Không có bonus đặc biệt, chỉ đủ giữ lò ổn định trong quá trình luyện đan.

```
/imagine prompt: a binding sealing cord wrapping a furnace with talisman knots, plain coarse cloth cord with simple knots, plain dull finish, simple thin gray border frame, no glow, neutral ivory-and-gold, centered game item icon on dark ink background, moody pixel art game art, xianxia daoist alchemy aesthetic, talismanic cinnabar runes and bagua trigram motifs, limited six-color palette of ink black, cinnabar red, imperial gold, ivory white, jade green and twilight purple, hand-crafted sprite craftsmanship in the spirit of Eastward and Sea of Stars, atmospheric serious mystical tone, crisp hard pixel edges --style raw --v 6 --no anime, manga, ukiyo-e, western fantasy, generic asian temple stock photo, photorealistic, 3d render, glossy plastic, modern, text, ui, watermark, signature, blur --ar 1:1 --stylize 120
```

---

### `belt-magic-elixir-flow` — Dây Phong Ấn Linh Đan Lưu

**Mô tả**: Dây lụa xanh phong ấn linh đan lưu — nút thắt khắc phù lưu đan, tăng ba mươi phần trăm drop Linh Đan toàn pháp trận. Ánh lam nhẹ toả ra từ các nút thắt talisman.

```
/imagine prompt: a binding sealing cord wrapping a furnace with talisman knots, blue silk cord with elixir-flow runes woven into each knot, soft blue inner glow, clean blue border frame, centered game item icon on dark ink background, [STYLE SUFFIX] --ar 1:1 --stylize 120
```

---

### `belt-rare-essence` — Dây Phong Ấn Bảo Tinh Hoa

**Mô tả**: Dây tinh hoa phẩm Bảo — vải gấm tím khảm hoa văn tinh hoa bảo thạch, nâng tier drop một cấp cho toàn bộ tinh hoa rơi trong pháp trận. Viền tím lấp lánh hoa văn bùa.

```
/imagine prompt: a binding sealing cord wrapping a furnace with talisman knots, brocade purple cord with essence-gem motifs at every knot, purple crystalline sheen, ornate purple border with a subtle repeating talisman pattern, centered game item icon on dark ink background, [STYLE SUFFIX] --ar 1:1 --stylize 120
```

---

### `belt-legendary-nine-turn` — Dây Phong Ấn Thần Cửu Chuyển

**Mô tả**: Dây cửu chuyển huyền thoại — chín nút thắt vàng xếp dọc, mỗi nút một vòng linh văn xoay. Cửu Chuyển Đan drop chance tăng năm mươi phần trăm. Ember cam tràn quanh dây như ngọn lửa phong ấn.

```
/imagine prompt: a binding sealing cord wrapping a furnace with talisman knots, nine golden knots in a row each engraved with a rotating nine-turn rune, bright orange aura with floating ember particles, glowing orange border frame, centered game item icon on dark ink background, [STYLE SUFFIX] --ar 1:1 --stylize 120
```

---

### `belt-heaven-earth-treasure` — Thiên Địa Bảo Đai

**Mô tả**: Cổ vật — Thiên Địa Bảo Đai, đai luyện đan thiêng liêng nhất — phong ấn không bao giờ bị brick trong toàn pháp trận, thất bại vẫn giữ nguyên affix. Nút thắt cầu vồng phát sáng, dây bện từ sợi thiên địa linh khí, hào quang hạt tử tứa ra toàn bộ chiều dài đai.

```
/imagine prompt: a binding sealing cord wrapping a furnace with talisman knots, the cord woven from heaven-and-earth spirit-threads with rainbow-shimmering fibers and iridescent knot ornaments, intense prismatic rainbow particle storm, shifting iridescent border, legendary radiance, centered game item icon on dark ink background, [STYLE SUFFIX] --ar 1:1 --stylize 140
```
